      ...
      <div className="rounded-xl w-[460px] shadow-2xl border themed-bg-secondary themed-border-primary" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b themed-border-primary">
          <div className="flex items-center gap-3">
            ...
            <h2 className={`text-base font-bold themed-text-primary`}>{ts('settings.title')}</h2>
          </div>
        ...
        <div className="p-5 space-y-5">
          {/* ===== 外观设置 ===== */}
          <div>
            <label className={`text-sm themed-text-secondary block mb-3 font-medium`}>{ts('settings.appearance')}</label>
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-sm themed-text-primary`}>{ts('settings.theme')}</span>
                </div>
                <div className="flex rounded-lg overflow-hidden border themed-border-primary">
                  ...
                  <button onClick={() => setTheme('light')}
                    className={`px-3 py-1.5 text-sm font-medium transition-colors ${theme === 'light'
                      ? 'themed-btn-primary'
                      : 'themed-text-muted themed-bg-hover'
                    }`}>
                    {ts('settings.light')}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-sm themed-text-primary`}>{ts('settings.language')}</span>
                </div>
                <div className="flex rounded-lg overflow-hidden border themed-border-primary">
                  <button onClick={() => setLocale('zh')}
                    className={`px-3 py-1.5 text-sm font-medium transition-colors ${locale === 'zh'
                      ? 'themed-btn-primary'
                      : 'themed-text-muted themed-bg-hover'
                    }`}>
                    中文
                  ...
                  <button onClick={() => setLocale('en')}
                    className={`px-3 py-1.5 text-sm font-medium transition-colors ${locale === 'en'
                      ? 'themed-btn-primary'
                      : 'themed-text-muted themed-bg-hover'
                    }`}>
                    EN
          ...
          {/* ===== 游戏目录 ===== */}
          <div>
            <label className={`text-sm themed-text-secondary block mb-2 font-medium`}>{ts('settings.gameDir')}</label>
            <p className={`text-xs themed-text-dimmed mb-3`}>
            ...
            <div className="rounded-lg p-3 mb-3 min-h-[40px] flex items-center themed-bg-primary">
              {gameDir ? (
                <div className="flex-1 min-w-0">
                  <span className={`text-sm themed-text-secondary inline-flex items-center gap-1`}>
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                    {ts('settings.set')}
                  </span>
                  <p className={`text-sm themed-text-primary mt-0.5 truncate`}>{gameDir}</p>
                </div>
              ) : (
            ...
            <div className="flex gap-3">
              <button onClick={handleAutoDetect} disabled={detecting}
                className="flex-1 px-4 py-2 text-sm rounded-lg transition-colors disabled:opacity-50 font-medium themed-btn-primary">
                {detecting
              ...
              <button onClick={handleManualSelect}
                className={`flex-1 px-4 py-2 text-sm rounded-lg transition-colors font-medium themed-btn-secondary`}>
                {ts('settings.manualSelect')}
              </button>
              {gameDir && (
                <button onClick={handleClear}
                  className={`px-3 py-2 text-sm themed-text-secondary hover:text-current rounded-lg themed-bg-hover transition-colors`}>
                  {ts('settings.clear')}
                </button>
              )}
            </div>
            ...
            {/* 状态提示 */}
            {statusMsg && (
              <p className={`text-xs mt-2 themed-text-dimmed`}>{statusMsg}</p>
            )}
          </div>
          ...
          {/* ===== 关于 ===== */}
          <div>
            <label className={`text-sm themed-text-secondary block mb-2 font-medium`}>{ts('settings.about')}</label>
            <div className={`space-y-1 text-xs themed-text-dimmed`}>
              <p>饭团工坊 v0.1.0</p>
        ...
        <div className="px-5 py-4 border-t flex justify-end themed-border-primary">
          <button onClick={onClose}
            className="px-5 py-2 text-base font-medium rounded-md transition-colors themed-btn-primary">
            {ts('settings.done')}
          </button>
        </div>
      </div>
