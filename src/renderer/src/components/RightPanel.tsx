...
const STORAGE_KEY = 'fantuan-workshop:rightPanelCollapsed'
...
export default function RightPanel({ currentPath }: { currentPath: string }): JSX.Element {
  ...
  if (collapsed) {
    return (
      <aside className="hidden xl:flex w-[40px] min-w-[40px] themed-bg-secondary flex-col border-l themed-border-secondary items-center pt-4">
        <button
          onClick={() => setCollapsed(false)}
          className="w-7 h-7 rounded-md themed-bg-hover themed-text-muted hover:themed-text-primary flex items-center justify-center transition-colors"
          title={ts('rightPanel.expand')}
        >
        ...
        {/* 竖排文字指示器 */}
        <div className="mt-6 [writing-mode:vertical-rl] text-[11px] themed-text-disabled tracking-wider select-none">
          {ts('rightPanel.currentPage')}
        </div>
      </aside>
    )
  }
    ...
    <aside className="hidden xl:flex w-[300px] min-w-[300px] themed-bg-secondary flex-col border-l themed-border-secondary overflow-y-auto">
      ...
      <div className="flex items-start justify-between px-5 pt-5 pb-3">
        <div className="min-w-0 flex-1">
          ...
          <h3 className="text-base font-medium themed-text-primary truncate">{pageTitle}</h3>
          {pageDesc && <p className="text-sm themed-text-dimmed mt-0.5">{pageDesc}</p>}
        </div>
        <button
          onClick={() => setCollapsed(true)}
          className="w-7 h-7 rounded-md themed-bg-hover themed-text-muted hover:themed-text-primary flex items-center justify-center flex-shrink-0 ml-2 transition-colors"
          title={ts('rightPanel.collapse')}
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
