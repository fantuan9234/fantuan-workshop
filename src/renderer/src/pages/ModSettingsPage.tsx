  ...
  return (
    <div className="p-4 md:p-8 h-full flex flex-col overflow-y-auto">
      ...
      <div className="flex items-center justify-between flex-shrink-0 mb-6">
        <div>
          <h2 className="text-3xl font-bold themed-text-primary">{ts('modSettings.title')}</h2>
          <p className="text-base themed-text-muted mt-1">{ts('modSettings.subtitle')}</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 mb-6 border-b themed-border-secondary pb-0 overflow-x-auto flex-shrink-0">
        {tabs.map(tab => (
          <button
            ...
            className={`px-4 py-2.5 text-base font-medium whitespace-nowrap transition-colors border-b-2 -mb-[1px] ${
              activeTab === tab.key
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="themed-bg-card rounded-xl p-5">
        <h3 className="text-base font-medium themed-text-secondary mb-3">{ts('modSettings.addConfig')}</h3>
        <div className="flex gap-3">
          ...
          />
          <button onClick={handleAdd} className="px-4 py-2 bg-[#07c160] hover:bg-[#06a850] text-white rounded-lg text-base font-medium transition-colors">
            {ts('modSettings.addBtn')}
          </button>
        </div>
        <p className="text-xs themed-text-dimmed mt-2">{ts('modSettings.configNameHint')}</p>
      </div>

      {/* 配置项列表 */}
      {entries.length === 0 ? (
        <div className="themed-bg-card rounded-xl p-8 text-center">
          <div className="themed-text-dimmed text-base">{ts('modSettings.noConfigEntries')}</div>
          <p className="text-sm themed-text-dimmed mt-1">{ts('modSettings.noConfigEntriesHint')}</p>
        </div>
      ) : (
            ...
            <div key={key} className="themed-bg-card rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <span className="px-2 py-0.5 rounded text-sm font-mono bg-[#07c160]/15 text-[#07c160]">{key}</span>
                  <span className="text-xs themed-text-dimmed">{'{{' + key + '}}'}</span>
                </div>
                <button onClick={() => handleDelete(key)} className="text-sm text-red-400 hover:text-red-300 transition-colors">
                ...
                <div className="flex items-end gap-4">
                  <Field label={ts('modSettings.configAllowBlank')}>
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      <span className="text-sm themed-text-muted">{ts('modSettings.configAllowBlankHint')}</span>
                    </label>
                  </Field>
                </div>
              </div>
              {entry.AllowRange && (
                <div className="mt-3 flex items-center gap-3">
                  <span className="text-sm themed-text-dimmed">{ts('modSettings.configAllowRange')}</span>
                  ...
                  <span className="text-sm themed-text-dimmed">~</span>
                  ...
                  <button onClick={() => handleUpdate(key, 'AllowRange', undefined)} className="text-xs text-red-400 hover:text-red-300">{ts('modSettings.removeBtn')}</button>
                </div>
              )}
              {!entry.AllowRange && entry.AllowValues && entry.AllowValues.split(',').some(v => !isNaN(Number(v.trim())) && v.trim() !== '') && (
                ...
                }} className="mt-2 text-xs text-[#07c160] hover:underline">{ts('modSettings.convertToRange')}</button>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="themed-bg-card rounded-xl p-4">
        <h3 className="text-base font-medium themed-text-secondary mb-2">{ts('modSettings.usageGuide')}</h3>
        <div className="text-sm themed-text-muted space-y-1 font-mono">
    ...
    <div className="space-y-4">
      ...
      <button onClick={handleAdd} className="w-full themed-bg-card rounded-xl p-5 text-left border border-dashed themed-border-active hover:border-[#07c160] transition-colors group">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-base font-medium themed-text-primary">{ts('modSettings.addToken')}</div>
            <div className="text-sm themed-text-muted mt-0.5">{ts('modSettings.addTokenDesc')}</div>
          </div>
        </div>
      </button>
      ...
      {dynamicTokens.length === 0 ? (
        <div className="themed-bg-card rounded-xl p-8 text-center">
          <div className="themed-text-dimmed text-base">{ts('modSettings.noTokens')}</div>
        </div>
      ) : (
        <div className="space-y-3">
          {dynamicTokens.map((token, index) => (
            <div key={index} className="themed-bg-card rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <span className="px-2 py-0.5 rounded text-sm font-mono bg-purple-500/15 text-purple-300">
                    {'{{' + (token.Name || '...') + '}}'}
                  </span>
                </div>
                <button onClick={() => handleDelete(index)} className="text-sm text-red-400 hover:text-red-300 transition-colors">
                  {ts('modSettings.deleteBtn')}
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="mt-3">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-sm themed-text-dimmed font-medium">When</span>
                  {token.When && Object.keys(token.When).map(wk => (
                    <span key={wk} className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs bg-amber-500/15 text-amber-300">
                      {wk}={token.When![wk]}
...
function WhenConditionsTab({ whenConditions, setWhenConditions, update }: {
  ...
  return (
    <div className="space-y-4">
      {/* 添加条件 */}
      <div className="themed-bg-card rounded-xl p-5">
        <h3 className="text-base font-medium themed-text-secondary mb-3">{ts('modSettings.addWhenCondition')}</h3>
        <div className="flex gap-3">
      ...
      {/* 快捷添加 */}
      <div className="themed-bg-card rounded-xl p-4">
        <h3 className="text-base font-medium themed-text-secondary mb-3">{ts('modSettings.quickAddCondition')}</h3>
        <div className="flex flex-wrap gap-3">
          {commonTokens.map(token => (
            <div key={token.key} className="flex items-center gap-1">
              <span className="text-xs themed-text-dimmed font-medium">{token.key}:</span>
      ...
      {entries.length === 0 ? (
        <div className="themed-bg-card rounded-xl p-8 text-center">
          <div className="themed-text-dimmed text-base">{ts('modSettings.noWhenConditions')}</div>
          <p className="text-sm themed-text-dimmed mt-1">{ts('modSettings.noWhenConditionsHint')}</p>
        </div>
      ) : (
        <div className="space-y-2">
          {entries.map(([key, value]) => (
            <div key={key} className="themed-bg-card rounded-lg p-3 flex items-center gap-3">
              <span className="px-2 py-0.5 rounded text-sm font-mono bg-amber-500/15 text-amber-300">{key}</span>
              <span className="text-sm themed-text-dimmed">=</span>
              <input type="text" value={value} onChange={e => handleUpdate(key, e.target.value)} className="input flex-1 text-base" />
              <button onClick={() => handleDelete(key)} className="text-sm text-red-400 hover:text-red-300 transition-colors flex-shrink-0">
                {ts('modSettings.deleteBtn')}
              </button>
            </div>
          ))}
      ...
      {/* 条件说明 */}
      <div className="themed-bg-card rounded-xl p-4">
        <h3 className="text-base font-medium themed-text-secondary mb-2">{ts('modSettings.whenConditionGuide')}</h3>
        <div className="text-sm themed-text-muted space-y-1">
    ...
    <div className="space-y-4">
      ...
      <div className="themed-bg-card rounded-xl p-5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-base font-medium themed-text-primary">{ts('modSettings.enableI18n')}</h3>
            <p className="text-sm themed-text-muted mt-1">{ts('modSettings.enableI18nHint')}</p>
          </div>
      ...
      {enableI18n && (
        <>
          {/* 语言管理 */}
          <div className="themed-bg-card rounded-xl p-5">
            <h3 className="text-base font-medium themed-text-secondary mb-3">{ts('modSettings.languageManage')}</h3>
            <div className="flex flex-wrap gap-3 mb-3">
              {languages.map(lang => (
                <div key={lang} className="flex items-center gap-1">
                  ...
                  {lang !== 'default' && (
                    <button onClick={() => handleDeleteLang(lang)} className="text-xs text-red-400 hover:text-red-300">&times;</button>
                  )}
                </div>
              ))}
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={handleAddLanguage} className="px-3 py-2 text-sm bg-[#07c160] hover:bg-[#06a850] text-white rounded-lg transition-colors">
          ...
          {/* 翻译条目 */}
          <div className="themed-bg-card rounded-xl p-5">
            <h3 className="text-base font-medium themed-text-secondary mb-3">
            ...
            <div className="flex gap-3 mb-3">
              ...
              <button onClick={handleAddEntry} className="px-3 py-2 text-sm bg-[#07c160] hover:bg-[#06a850] text-white rounded-lg transition-colors">
                {ts('modSettings.addBtn')}
              </button>
            </div>
            <div className="space-y-2 max-h-[400px] overflow-y-auto">
              {Object.entries(currentEntries).map(([key, value]) => (
                <div key={key} className="flex items-center gap-3">
                  <span className="px-2 py-0.5 rounded text-xs font-mono bg-blue-500/15 text-blue-300 flex-shrink-0 max-w-[200px] truncate">{key}</span>
                  <input type="text" value={value} onChange={e => handleUpdateEntry(key, e.target.value)} className="input flex-1 text-base" />
                  <button onClick={() => handleDeleteEntry(key)} className="text-xs text-red-400 hover:text-red-300 flex-shrink-0">&times;</button>
                </div>
              ))}
              {Object.keys(currentEntries).length === 0 && (
                <div className="text-center py-4 text-sm themed-text-dimmed">{ts('modSettings.noI18nEntries')}</div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="themed-bg-card rounded-xl p-5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-base font-medium themed-text-primary">{ts('modSettings.splitContentFiles')}</h3>
            <p className="text-sm themed-text-muted mt-1">{ts('modSettings.splitContentFilesHint')}</p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
        ...
        {splitContentFiles && (
          <div className="mt-3 p-3 themed-bg-primary rounded-lg">
            <p className="text-sm themed-text-muted">{ts('modSettings.splitContentFilesDesc')}</p>
          </div>
        )}
      </div>
      ...
      {/* 补丁优先级 */}
      <div className="themed-bg-card rounded-xl p-5">
        <h3 className="text-base font-medium themed-text-primary mb-3">{ts('modSettings.patchPriority')}</h3>
        <p className="text-sm themed-text-muted mb-3">{ts('modSettings.patchPriorityHint')}</p>
        <div className="flex gap-3">
          {(['Low', 'Normal', 'High'] as const).map(p => (
            <button
              key={p}
              onClick={() => update('patchPriority', () => { setPatchPriority(p) })}
              className={`flex-1 px-4 py-2.5 rounded-lg text-base font-medium transition-colors ${
                patchPriority === p
      ...
      {/* Include 说明 */}
      <div className="themed-bg-card rounded-xl p-4">
        <h3 className="text-base font-medium themed-text-secondary mb-2">{ts('modSettings.includeGuide')}</h3>
        <div className="text-sm themed-text-muted space-y-1 font-mono">
