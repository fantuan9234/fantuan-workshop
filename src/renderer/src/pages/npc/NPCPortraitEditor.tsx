  ...
  return (
    <div className="p-4 md:p-8 h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-4 flex-shrink-0">
        ...
        <span className="text-base text-white font-medium">编辑肖像</span>
        {customCount > 0 && (
          <span className="text-sm px-2 py-0.5 rounded-full bg-[#3a3a3a] text-gray-400">
            {customCount} 项已替换
          </span>
        )}
      ...
      {!gameDir && (
        <div className="mb-4 bg-[#1a1a1a] border border-[#444] rounded-lg p-4 flex items-start gap-3 flex-shrink-0">
          ...
          <div>
            <p className="text-base text-gray-300 font-medium">肖像需要从你的星露谷游戏目录中读取</p>
      ...
      <div className="flex-1 overflow-y-auto space-y-8 pb-4">
        ...
        {!showAddPanel ? (
          <button
            ...
            className="w-full border border-dashed border-[#444] rounded-xl p-4 flex items-center justify-center gap-3 text-base text-gray-400 hover:border-[#666] hover:text-gray-300 transition-colors"
          >
              ...
              {sceneTypes.map((scene, idx) => (
                !activeScenes.includes(idx) && (
                  <button
                    ...
                    className="text-sm px-3 py-1.5 rounded-md bg-[#2a2a2a] text-gray-300 hover:bg-[#3a3a3a] transition-colors"
                  >
                    {scene}
                  </button>
                )
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-medium text-white">{scene}</h3>
        <div className="flex items-center gap-3">
          {canRemove && onRemove && (
            <button
              onClick={onRemove}
              className="text-sm text-gray-500 hover:text-red-400 px-2 py-1 rounded hover:bg-[#3a3a3a] transition-colors"
            >
              移除
            </button>
          )}
          {hasCustoms && (
            <button
              onClick={onResetAll}
              className="text-sm text-gray-400 hover:text-white px-2 py-1 rounded hover:bg-[#3a3a3a] transition-colors"
            >
              重置本组
            </button>
          )}
          <button className="relative text-sm text-gray-400 hover:text-white px-3 py-1.5 rounded border border-[#3a3a3a] hover:border-[#555] transition-colors overflow-hidden">
            替换整张
            ...
            return (
              <div key={exprIdx}
                ...
                <div className="absolute top-1 right-1 flex gap-1">
                  {isCustom && (
                    <span className="text-[11px] px-1 py-0.5 rounded bg-[#555] text-white leading-none inline-flex items-center">
                ...
                {isCustom && (
                  <button
                    ...
                    className="absolute top-1 left-1 text-[11px] px-1 py-0.5 rounded bg-[#555] text-white leading-none opacity-0 group-hover:opacity-100 transition-opacity z-20"
                  >
                    重置
                  </button>
                )}
                ...
                <div className="py-1.5 px-1 text-center w-full bg-[#1a1a1a] flex-shrink-0">
                  <div className="text-xs text-gray-300 truncate">{expressionLabels[String(exprIdx)]}</div>
                  <div className="text-[11px] text-gray-600">#{exprIdx}</div>
                </div>
              </div>
            )
          })}
...
function CustomPortraitSceneGroup({
  ...
  return (
    <div>
      {/* 场景标题 + 操作按钮 */}
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-medium text-white">{sceneName}</h3>
        <div className="flex items-center gap-3">
          {canRemove && (
          ...
          <button onClick={onSelect}
            className="text-sm text-gray-400 hover:text-white px-3 py-1.5 rounded border border-[#3a3a3a] hover:border-[#555] transition-colors">
            替换整张
          </button>
          {isUnpackPreview && (
            <span className="text-[11px] px-1.5 py-0.5 rounded bg-[#333] text-gray-500">参考预览</span>
          )}
        </div>
      </div>
            ...
            return (
              <div key={exprIdx}
                ...
                <div className="py-1 px-0.5 text-center w-full bg-[#1a1a1a]">
                  <div className={`text-[11px] truncate ${displayUrl && !isUnpackPreview ? 'text-gray-300' : 'text-gray-500'}`}>{expressionLabels[String(exprIdx)]}</div>
      ...
      {portraitUrl && (
        <div className="relative rounded-xl overflow-hidden group max-w-[200px] border-2 border-[#555] bg-[#1e1e1e]">
          ...
          <button onClick={(e) => { e.stopPropagation(); onDelete() }}
            className="absolute top-2 right-2 text-sm px-2 py-1 rounded bg-red-900/80 text-red-300 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-800">
            删除
          </button>
        </div>
      )}
